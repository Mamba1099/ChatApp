<?php 
    ...
    if(!empty($email) && !empty($password)){ // If email and password is not empty
        $sql = mysqli_query($conn, "SELECT * FROM users WHERE email = '{$email}'"); // Checking if email exists in database
        if(mysqli_num_rows($sql) > 0){ // If email exists
            ...
            if($user_pass === $enc_pass){
                ...
                $sql2 = mysqli_query($conn, "UPDATE users SET status = '{$status}' WHERE unique_id = {$row['unique_id']}"); // Updating status in database
                if($sql2){

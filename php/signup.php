 ...
 */
    ...
    $fname = mysqli_real_escape_string(
        $conn, 
        $_POST['fname']
    ); // Getting fname from post variable
    $lname = mysqli_real_escape_string(
        $conn, 
        $_POST['lname']
    ); // Getting lname from post variable
                    ...
                    if (in_array($img_ext, $extensions) === true) { 
                        ...
                        if (in_array($img_type, $types) === true) { 
                            ...
                            if (move_uploaded_file(
                                ...
                                // If image moved to our folder
                                $ran_id = rand(time(), 100000000); 
                                // Creating random id for user
                                $status = "Active now"; 
                                // after sign up status active
                                ...
                                if ($insert_query) {
                                    ...
                                    if (mysqli_num_rows($select_sql2) > 0) { 
                                        // If user data is there in database
                                        $result = mysqli_fetch_assoc($select_sql2); 
                                        // Fetching user data
                                        $_SESSION[
                                            'unique_id'
                                            ] = $result['unique_id']; 
                                        // Putting user id into session variable
                                        echo "success";
                                    } else {
